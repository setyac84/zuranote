...
const TaskListPage = () => {
  ...
  if (memberFilter) {
    const m = allMembers.find(u => u.id === memberFilter);
    title = `Tasks by ${m?.name || ''}`;
  }
  if (priorityFilter) title = 'High Priority Tasks';
  if (projectFilter) {
    const p = allProjects.find(pr => pr.id === projectFilter);
    title = `Tasks: ${p?.name || ''}`;
  }
          ...
          {filteredTasks.map((task, i) => {
            ...
            return (
              <motion.div key={task.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                onClick={() => setSelectedTask(task)} className="glass-card rounded-xl p-4 cursor-pointer hover:border-primary/30 transition-all overflow-visible">
